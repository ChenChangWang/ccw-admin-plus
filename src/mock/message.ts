...
import type { MockParams } from "./constants.ts";
import { successResponse } from "@/mock/response-wrap";
import { MessageData, NoticeData, TodoData } from "@/api/message.ts";
...
Mock.mock("/api/message/messageRead", (params: MockParams) => {
  const { body } = params;
...
Mock.mock("/api/message/noticeRead", (params: MockParams) => {
  const { body } = params;
  const { id } = JSON.parse(body);
...
Mock.mock("/api/message/todoRead", (params: MockParams) => {
  const { body } = params;
  const { id } = JSON.parse(body);
