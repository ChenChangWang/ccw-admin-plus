...
import { MockParams } from "./constants.ts";
import { successResponse } from "@/mock/response-wrap";
import dayjs from "dayjs";
import type { AxisData, ShopData } from "@/api/dashboard.ts";
import {
  SalesProportionData,
...
Mock.mock("/api/querySearchUser", () => {
  return successResponse(searchUserData);
});
...
Mock.mock("/api/querySalesProportion", (params: MockParams) => {
  const { body } = params;
