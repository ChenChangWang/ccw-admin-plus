...
Mock.mock("/api/list/searchTable", (params) => {
  const { body } = params;
...
Mock.mock("/api/list/basicList", (params) => {
  const { body } = params;
  const { currentPage, pageSize } = JSON.parse(body);
  const startIndex = (currentPage - 1) * pageSize;
  const endIndex = currentPage * pageSize;
...
Mock.mock("/api/list/basicAdd", (params) => {
  const { body } = params;
...
Mock.mock("/api/list/basicEdit", (params) => {
  const { body } = params;
  const data = JSON.parse(body);
...
Mock.mock("/api/list/basicDelete", (params) => {
  const { body } = params;
  const data = JSON.parse(body);
