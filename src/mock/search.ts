...
import { ApplicationData, ArticleData, ProjectData } from "@/api/search.ts";

// =========================== 文章 =======================
const articleBaseList: ArticleData[] = [
  {
    title: "Vue",
    content:
...
Mock.mock("/api/search/articleList", (params: MockParams) => {
  const { body } = params;
...
// =========================== 项目 =======================
const projectsBaseList: ProjectData[] = [
  {
    title: "Vue",
...
Mock.mock("/api/search/projectList", (params: MockParams) => {
  const { body } = params;
  const { currentPage, pageSize } = JSON.parse(body);
  const startIndex = (currentPage - 1) * pageSize;
  const endIndex = currentPage * pageSize;
...
// =========================== 应用 =======================
const applicationBaseList: ApplicationData[] = [
  {
    title: "Vue",
    newUser: 1605,
    activeUser: 163258,
...
let applicationList: ApplicationData[] = [];
...
Mock.mock("/api/search/applicationList", (params: MockParams) => {
  const { body } = params;
  const { currentPage, pageSize } = JSON.parse(body);
  const startIndex = (currentPage - 1) * pageSize;
  const endIndex = currentPage * pageSize;
